...
const ADD_FIELD_ICON: Record<FieldTypeValue, React.ComponentType<{ className?: string }>> = {
  ...
  // unused here, present only to satisfy the Record type
...
export const AddFieldPanel = ({ onAddField }: AddFieldPanelProps) => {
  return (
    <div className="flex flex-col gap-6 items-start w-full">
      <p className="text-grey-700 text-lg font-medium">Add Field</p>
      <div className="flex flex-wrap gap-4 w-full">
        {ADDABLE_FIELD_TYPES.map((type) => {
          ...
          return (
            ...
            >
              <Icon className="size-5 text-grey-700" />
              <span className="text-grey-700 text-base">{FIELD_TYPE_META[type].label}</span>
            </button>
          );
        })}
      </div>
